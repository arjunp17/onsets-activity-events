...
def sed_cond_sad(rows,cols,channels,nb_classes,lr):
...
###############################################################################################
	...
	# aggregation
	concat = concatenate([sl_out_int, pa_out], axis=2)
	...
	model = Model(input, outputs = [sl_out,pa_out], name='sed_sad')
	# compile model
	opt = Adam(lr = lr)
	losses = {'sl_out': 'binary_crossentropy','pa_out': 'binary_crossentropy'}
	lossWeights = {'sl_out': 0.5, 'pa_out': 0.5}
	model.compile(loss=losses, loss_weights=lossWeights, optimizer=opt, metrics=['accuracy'])
	model.summary()
	return model


sed_cond_sad = sed_cond_sad(rows,cols,channels,nb_classes,lr)
...
train_history = model_train(sed_cond_sad,X_train,Y_train_SED,Y_train_SAD,X_val,Y_val_SED,Y_val_SAD,epochs,batch_size,sed_sad,output_folder)
